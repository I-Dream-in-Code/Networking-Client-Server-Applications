...
struct server_request_join {
        request_t req_type; /* = SERVER_REQ_JOIN */
        char req_channel[CHANNEL_MAX];
} packed;

struct server_request_leave {
        request_t req_type; /* = SERVER_REQ_LEAVE */
        char req_channel[CHANNEL_MAX];
} packed;

struct server_request_say {
        request_t req_type; /* = SERVER_REQ_SAY */
        char uuid_str[37];
        char req_username[USERNAME_MAX];
        char req_channel[CHANNEL_MAX];
        char req_text[SAY_MAX];
